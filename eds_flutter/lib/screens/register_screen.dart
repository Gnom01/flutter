...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    final result = await _authService.sendSmsOtp(_phone);
  ...
  Future<void> _register() async {
    ...
    setState(() => _isLoading = true);
    final result = await _authService.registerAccount(
      phone: _phone,
      otpToken: _otpToken,
    ...
    if (result['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Konto zostało utworzone! Możesz się zalogować.'),
          backgroundColor: Colors.green.shade600,
        ),
      );
      Navigator.of(context).pop();
    } else {
      _showError(result['message'] ?? 'Błąd rejestracji');
    }
  }
  ...
  Widget _buildStepIndicator() {
    const labels = ['Telefon', 'Kod SMS', 'Dane'];
    return Padding(
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Wyślemy Ci jednorazowy kod SMS, aby potwierdzić Twoją tożsamość.',
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            validator: (v) => (v == null || v.isEmpty) ? 'Wprowadź imię' : null,
          ),
          const SizedBox(height: 16),
          TextFormField(
